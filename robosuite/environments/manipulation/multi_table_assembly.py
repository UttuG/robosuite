...
from robosuite.models.objects.composite import CubeConeObject
...
class MultiTableAssembly(ManipulationEnv):
    """
    This class corresponds to the multi-table assembly task for a single robot arm.
    
    The robot must pick assembly items from a source table and stack them on top of
    base objects located on a destination table, simulating warehouse assembly workflows.
    ...
    Args:
        ...
        tables_boundary (3-tuple): x, y, and z dimensions of the multi-table workspace boundary.
            Tables will be positioned within this boundary.
    ...
    def __init__(
        ...
        # settings for table top - use smaller tables for close proximity
        self.tables_boundary = tables_boundary
        self.table_full_size = np.array(tables_boundary)
        self.table_full_size[1] *= 0.25  # each table size is a fraction of the full boundary
        self.table_friction = table_friction
        
        # Setup two tables close together for single arm reach
        self.table_offsets = np.zeros((2, 3))
        self.table_offsets[0, 1] = self.tables_boundary[1] * -3 / 8  # source table (left)
        self.table_offsets[1, 1] = self.tables_boundary[1] * 3 / 8   # destination table (right)
        self.table_offsets[:, 2] = 0.8  # table height

        # reward configuration (same as Stack)
    ...
    def reward(self, action):
        ...
        Sparse un-normalized reward:
            ...
            - a discrete reward of 2.0 is provided if the item from source table is stacked on the base on destination table
    ...
    def staged_rewards(self):
        ...
        """
        # reaching is successful when the gripper site is close to the center of the item cube
    ...
    def _load_model(self):
        ...
        # Adjust base pose accordingly - position robot between the two tables
        xpos = self.robots[0].robot_model.base_xpos_offset["table"](self.table_full_size[0])
        self.robots[0].robot_model.set_base_xpos(xpos)

        # load model for multi-table workspace
        ...
        # Modify default agentview camera to see both tables and backdrop
        mujoco_arena.set_camera(
            camera_name="agentview",
            pos=[0.8, 0, 1.4],
            quat=[0.6530981, 0.2710406, 0.2710408, 0.6530980],
        )

        # initialize objects of interest - similar to Stack
        tex_attrib = {
            "type": "cube",
        }
        mat_attrib = {
        ...
        # Create placement initializer - separate samplers for each table
        if self.placement_initializer is not None:
            self.placement_initializer.reset()
            self.placement_initializer.add_objects(cubes)
        else:
            # We'll create separate samplers for each cube on different tables
            # This will be handled in _reset_internal
            pass
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            if self.placement_initializer is not None:
                ...
                # Use separate samplers for cubeA (source table) and cubeB (destination table)
                # cubeA on source table (table 0) - reduced range to avoid edges
                cubeA_sampler = UniformRandomSampler(
                    name="CubeASampler",
                ...
                # cubeB on destination table (table 1) - reduced range to avoid edges
                cubeB_sampler = UniformRandomSampler(
                    name="CubeBSampler",
                ...
                cubeA_placements = cubeA_sampler.sample()
                cubeB_placements = cubeB_sampler.sample()
                
                for obj_pos, obj_quat, obj in cubeA_placements.values():
                    self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))
                
                for obj_pos, obj_quat, obj in cubeB_placements.values():
                    self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            # position and rotation of the first cube (item to pick)
